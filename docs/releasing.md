...
## Phase B v0.2.0 Release Notes

Phase B v0.2.0 is the current package-surface cleanup release candidate for
...
Phase B v0.2.0 does not include:
...
```bash
dotnet nuget push artifacts/packages/sc-lint-roslyn-demagic.0.2.0.nupkg \
  --source https://api.nuget.org/v3/index.json \
  --api-key "$NUGET_API_KEY" \
  --skip-duplicate

dotnet nuget push artifacts/packages/sc-lint-roslyn.0.2.0.nupkg \
  --source https://api.nuget.org/v3/index.json \
  --api-key "$NUGET_API_KEY" \
  --skip-duplicate
```
...
3. `dotnet tool install --global sc-lint-roslyn --version 0.2.0`
