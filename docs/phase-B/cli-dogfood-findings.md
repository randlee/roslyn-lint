...
| --- | --- | --- | --- | --- |
| `version --json` | `dotnet run --no-build --project src/sc.lint.roslyn/sc.lint.roslyn.csproj --framework net10.0 -- version --json` | Returns a clean success envelope with `command: "version"` and `data.cli = "sc-lint-roslyn"`, `data.version = "0.2.0.0"` | yes | `immediate-fix` |
