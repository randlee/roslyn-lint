# sc-lint-roslyn Boundary Inventory
...
- owns the top-level command families presented by `sc-lint-roslyn`
...
- `sc-lint-roslyn lint demagic` maps to `lint.demagic`
- `sc-lint-roslyn lint fast` maps to `lint.fast`
...
- backend-native top-level keys must not leak into the public `sc-lint-roslyn`
  envelope
...
- owns the approved shipping model for the `sc.lint.roslyn` executable package
