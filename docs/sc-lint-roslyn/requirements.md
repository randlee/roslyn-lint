# sc-lint-roslyn Requirements

## 1. Product Definition

`sc-lint-roslyn` is the suite CLI project and the stable top-level executable for
the repository's planned lint-tool family.

It is not a one-off wrapper around `sc.lint.roslyn.demagic`. It is the umbrella
...
- `REQ-CLI-PRODUCT-001` `sc-lint-roslyn` remains a first-class project in this
  repository distinct from `sc.lint.roslyn.demagic`.
...
- `REQ-CLI-PACK-001` `sc.lint.roslyn` is planned to ship as a .NET tool package,
  not as a library-only package.
- `REQ-CLI-PACK-002` The packaged tool command name remains `sc-lint-roslyn`.
...
- `REQ-CLI-PACK-004` The publish workflow must pack and publish the same
  `sc-lint-roslyn` .NET tool package shape described by repo docs and local pack
  commands.
...
- `REQ-CLI-EXT-001` The suite reserves one low-level shared package named
  `sc.lint.roslyn.abstractions` for tool-module integration contracts.
- `REQ-CLI-EXT-002` `sc.lint.roslyn.abstractions` is the planned home for shared
...
- `REQ-CLI-EXT-004` Custom attributes are allowed only after a concrete
  ...
  be insufficient.
- `REQ-CLI-EXT-005` `sc.lint.roslyn.Core` is not part of the baseline plan and
...
- `REQ-CLI-SURFACE-003` Top-level `sc-lint-roslyn ci` must remain distinct from
  `sc-lint-roslyn lint ci`; the first is lint-plus-tests orchestration and the
...
- `REQ-CLI-SURFACE-005` The primary analyzer entry path must be
  `sc-lint-roslyn lint <tool>`, with `demagic` as the first documented
  tool identifier.
