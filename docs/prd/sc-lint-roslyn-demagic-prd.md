# PRD: sc.lint.roslyn.demagic

**Product:** `sc.lint.roslyn.demagic`  
**Suite:** `sc-lint-roslyn`  
...
`sc.lint.roslyn.demagic` is the first analyzer in the `sc-lint-roslyn` suite. It enforces two classes of code hygiene rules that prevent domain coupling and structural decay in C# codebases: constant consolidation and forbidden string literal values.
...
Both failures are invisible to standard Roslyn analyzers. `sc.lint.roslyn.demagic` makes them visible and enforceable.
...
- Config-driven: all rule behavior defined in `.sc-lint-roslyn/config-src.toml`
  or `.sc-lint-roslyn/config-test.toml`
...
All rule configuration lives in `.sc-lint-roslyn/` at the solution root. There are two independent config files — one for src projects, one for test projects. Each is fully self-contained; there is no inheritance or merging between them.
...
<ItemGroup>
  <AdditionalFiles
      Include="$(MSBuildThisFileDirectory).sc-lint-roslyn\config-src.toml"
      Condition="Exists('$(MSBuildThisFileDirectory).sc-lint-roslyn\config-src.toml')
                 AND '$(IsTestProject)' != 'true'" />
  <AdditionalFiles
      Include="$(MSBuildThisFileDirectory).sc-lint-roslyn\config-test.toml"
      Condition="Exists('$(MSBuildThisFileDirectory).sc-lint-roslyn\config-test.toml')
                 AND '$(IsTestProject)' == 'true'" />
</ItemGroup>
```
...
Both files share the same schema. All `sc-lint-roslyn` analyzers add their sections to these files — no additional `AdditionalFiles` wiring is needed when new analyzers are added.

```toml
# .sc-lint-roslyn/config-src.toml  (or config-test.toml — same schema)
...
**Diagnostic ID:** `DM001`  
**Category:** `sc.lint.roslyn.organization`  
**Default Severity:** Warning

#### Description

Public and internal `const` field declarations must be declared in the
designated constants file/class as configured in `.sc-lint-roslyn/config-src.toml`
or `.sc-lint-roslyn/config-test.toml`. Constants found in any other file trigger
this diagnostic.
...
| Assembly | `sc.lint.roslyn.demagic.dll` |
...
`roslyn-diff` and `roslyn-graph` are candidates for absorption as supporting infrastructure once `sc-lint-roslyn` matures. DM001 in particular could leverage graph-based constant reference analysis in a future version for smarter fix suggestions.
