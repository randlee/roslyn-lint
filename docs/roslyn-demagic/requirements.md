...
`Roslyn.DeMagic` is the first analyzer package in the `roslyn-lint` suite.

This document turns the PRD in `docs/prd/roslyn-demagic-prd.md` into the
approved requirements baseline for implementation and review.

Product requirement IDs:

- `REQ-DM-PRODUCT-001` `Roslyn.DeMagic` must ship as a Roslyn analyzer package
...
- `REQ-DM-CONFIG-001` Analyzer behavior must be driven by files under
  `.roslyn-lint/` at the solution root.
- `REQ-DM-CONFIG-002` Source projects use `.roslyn-lint/config-src.toml`.
- `REQ-DM-CONFIG-003` Test projects use `.roslyn-lint/config-test.toml`.
...
- `REQ-DM-DIAG-005` Diagnostic categories must reflect the approved product
  ...
  `roslyn-lint.Organization` for `DM001` and `roslyn-lint.DomainBoundary` for
  `DM002`.
...
- `REQ-DM-PACK-001` The package ID is `Roslyn.DeMagic`.
