...
public sealed class RunCiOperationTests
{
    ...
    public async Task ExecuteAsync_RunsLintProfileAndTests()
    {
        ...
        var runner = new StubRunner(
            new DotnetCommandResult("/repo", ["test", "tests/sc.lint.roslyn.demagic.tests/sc.lint.roslyn.demagic.tests.csproj"], 0, string.Empty, string.Empty),
            new DotnetCommandResult("/repo", ["test", "tests/sc.lint.roslyn.tests/sc.lint.roslyn.tests.csproj"], 0, string.Empty, string.Empty));
        ...
        runner.Invocations[0].Should().Contain("tests/sc.lint.roslyn.demagic.tests/sc.lint.roslyn.demagic.tests.csproj");
        runner.Invocations[1].Should().Contain("tests/sc.lint.roslyn.tests/sc.lint.roslyn.tests.csproj");
    }
    ...
    public async Task ExecuteAsync_WhenTestFails_ThrowsDotnetCommandFailedException()
    {
        ...
        var runner = new StubRunner(
            new DotnetCommandResult("/repo", ["test", "tests/sc.lint.roslyn.demagic.tests/sc.lint.roslyn.demagic.tests.csproj"], 0, string.Empty, string.Empty),
            new DotnetCommandResult("/repo", ["test", "tests/sc.lint.roslyn.tests/sc.lint.roslyn.tests.csproj"], 1, string.Empty, "boom"));
        var operation = new RunCiOperation(lintOperation, runner);
